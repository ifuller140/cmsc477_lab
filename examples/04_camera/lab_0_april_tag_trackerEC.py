...
def set_velocity(t_ca, R_ca):
    # Simple P controller to calculate velocity command to move towards the AprilTag
    Kp_x = 2
    Kp_y = 2
    if (t_ca[2]-0.5) > 0:
        x_velocity = min(0.5, Kp_x * (t_ca[2]-0.5)) # Move forward/backward based on the z distance to the tag
    else: 
        x_velocity = max(-0.5, Kp_x * (t_ca[2]-0.5))
    print("x_velocity: " + str(x_velocity))
    y_velocity = Kp_y * t_ca[0] # Move left/right based on the x distance to the tag
