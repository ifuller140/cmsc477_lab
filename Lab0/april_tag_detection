...
def set_velocity(t_ca, R_ca):
    # PI controller to calculate velocity command to move towards the AprilTag
    Kp_x = 0.8  # Proportional gain for x (forward/backward)
    Ki_x = 0.1  # Integral gain for x
    Kp_y = 0.8  # Proportional gain for y (left/right)
    Ki_y = 0.1  # Integral gain for y

    # Static target velocity for the AprilTag
    target_velocity = 0.5  # m/s

    # Initialize integral terms (persistent across calls)
    if not hasattr(set_velocity, "integral_x"):
        set_velocity.integral_x = 0.0
    if not hasattr(set_velocity, "integral_y"):
        set_velocity.integral_y = 0.0

    # Calculate errors
    error_x = target_velocity - t_ca[2]  # Error in forward/backward direction
    error_y = -t_ca[0]  # Error in left/right direction (negative for correction)

    # Update integral terms
    set_velocity.integral_x += error_x
    set_velocity.integral_y += error_y

    # Calculate control signals
    x_velocity = Kp_x * error_x + Ki_x * set_velocity.integral_x
    y_velocity = Kp_y * error_y + Ki_y * set_velocity.integral_y

    # Send velocity commands to the robot
    ep_chassis.drive_speed(x=x_velocity, y=y_velocity, z=0, timeout=0.5)

    # Print the robot's velocity for monitoring
    print(f"Robot Velocity -> x: {x_velocity:.2f} m/s, y: {y_velocity:.2f} m/s")
